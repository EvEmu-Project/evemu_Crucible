...
static void _Jv_##_name (int _dummy __attribute__ ((__unused__))/*, siginfo_t *siginfo, void *context*/)
...
void sigexcept_restore (void) asm ("__restore");


#ifdef USE_GLIBC_SIGACT
#define INIT_GENERIC_SIGNAL(pname, signum)					\
do							\
  {							\
    struct sigaction act;		\
	memset(&act, 0, sizeof(act)); \
    act.sa_handler = _Jv_##pname;			\
    act.sa_flags = SA_NODEFER/* | SA_SIGINFO*/;			\
    act.sa_restorer = sigexcept_restore;				\
    sigaction(signum, &act, NULL);	\
  }							\
while (0) 

#define INIT_SEGV INIT_GENERIC_SIGNAL(catch_segv, SIGSEGV)
#define INIT_FPE INIT_GENERIC_SIGNAL(catch_fpe, SIGFPE)
#define INIT_ILL INIT_GENERIC_SIGNAL(catch_ill, SIGILL)
...
do							\
  {							\
    ...
    kact.k_sa_handler = _Jv_catch_segv;			\
    kact.k_sa_mask = 0;					\
    kact.k_sa_flags = SA_NODEFER;			\
    kact.sa_restorer = sigexcept_restore;				\
...
do							\
  {							\
    struct old_i386_kernel_sigaction kact;		\
    kact.k_sa_handler = _Jv_catch_fpe;			\
    kact.k_sa_mask = 0;					\
    kact.k_sa_flags = SA_NODEFER;			\
    kact.sa_restorer = sigexcept_restore;				\
...
do							\
  {							\
    struct old_i386_kernel_sigaction kact;		\
    kact.k_sa_handler = _Jv_catch_ill;			\
    kact.k_sa_mask = 0;					\
    kact.k_sa_flags =  SA_NODEFER;			\
    kact.sa_restorer = sigexcept_restore;				\
    syscall (SYS_sigaction, SIGILL, &kact, NULL);	\
  }							\
while (0)   
