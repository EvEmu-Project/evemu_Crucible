...
public:
    ...
    explicit counted_ptr(X* p = NULL) // allocate a new counter
        : itsCounter(NULL) {itsCounter = new counter(p);}
    ~counted_ptr()
        {release();}
	...
	//add a means to extract the underlying pointer, and swap in NULL
	X *hijack() { X *p = itsCounter->ptr; itsCounter->ptr = NULL; return(p); }
