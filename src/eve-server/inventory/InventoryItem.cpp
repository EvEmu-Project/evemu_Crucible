...
void InventoryItem::GetItemStatusRow( PyPackedRow* into ) const
{
	...
	into->SetField( "online",        new PyBool( (mAttributeMap.HasAttribute(AttrIsOnline) ? (GetAttribute(AttrIsOnline).get_int() == 1 ? 1 : 0) : 0) ) );
    ...
    into->SetField( "incapacitated", new PyBool( (mAttributeMap.HasAttribute(AttrIsIncapacitated) ? (GetAttribute(AttrIsIncapacitated).get_int() == 1 ? 1 : 0) : 0) ) );
}
