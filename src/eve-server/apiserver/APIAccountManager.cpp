...
std::tr1::shared_ptr<std::string> APIAccountManager::_AccountStatus(const APICommandCall * pAPICommandCall)
{
    ...
    {
        sLog.Error( "APIAccountManager::_AccountStatus()", "ERROR: Could not find 'accountID' in 'account' table; there is an invalid 'accountID' referenced by api account 'userID' = %i - exiting with error", accountID );
        return BuildErrorXMLResponse( "106", "Must provide userID parameter for authentication." );
    }
