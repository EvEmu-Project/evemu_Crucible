...
for region in range(0,regionID_List_length):
    ...
    for index in range(0,10): #range(1,typeID_List_length-1)
        ...
        if (contents != "The request contains bad syntax or cannot be fulfilled."):
                        ...
                        for f in e.getchildren():
                            ...
                            if ((e.tag == "sell") and (f.tag == "avg")):
                                if (option == options[0]):
                                    ...
                                    print "[%.2f" % typeID_Progress, "%] ", "typeID", int(typeID), "[", typeName_List[index], "]  BASE PRICE UPDATED !"
